...
//        }
    ...
    public void cullSpecies(boolean cutToOne) {
        ...
        for (Species s : this.Species) {
            ...
            while (s.Genomes.size() > remain) {
                s.Genomes.remove(0);//s.Genomes.size() - 1);
    ...
    public void save(String s, int i) {
        try {
            ...
            out.close();
            fileOut.close();
            System.out.println("Serialized data is saved in " + f.getAbsolutePath() + " with Generation " + this.generation + " Fitness " + this.getbest().getFitness());
        }

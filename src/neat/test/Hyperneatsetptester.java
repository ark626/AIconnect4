...
public class Hyperneatsetptester {
    ...
    public void test() {
        ...
        Genome g = null;
        for (int k = 0; k < 10; k++) {
            g = test.getPool().Species.get(test.getPool().currentSpecies - 1).Genomes.get(test.getPool().currentGenome - 1);
            g.generateNetwork();
            test.generateweigths(g);
            while (test.getPool().alreadyMeasured()) {
                g = test.getPool().Species.get(test.getPool().currentSpecies - 1).Genomes.get(test.getPool().currentGenome - 1);
                g.generateNetwork();
                test.generateweigths(g);
            }
            for (int z = 1; z < 1000; z++) {
                double[] in = randInput(r, test.getInput());

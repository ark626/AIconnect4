        	...
        	try {
    		   ...
    		   //       this.g.generateNetwork();
    		    //List Neurons
    		          ...
    		          int z = 0;
    		          int zähler = 0;
    		          for(int i = 0;i<this.g.getNetwork().Neurons.size();i++){
    		        	  if(i<this.g.getInputs()/6){
    		        		  for(z = 0;z<6;z++){
    		        			  Nodes.add(20.0+25*i);
    		    		          Nodes.add(100.0+25*z);
    		    		          Nodes.add(this.g.getNetwork().Neurons.get(zähler++).getValue());
    		        	  }
    		        	  }
    		          }
    		          
    		          for(int i =0;i<this.g.getNetwork().Neurons.size();i++){
    		        	  Neuron n = this.g.getNetwork().Neurons.get(i);
    		        	  if(i<this.g.getInputs()){

    		        	  }
    		        	  else{
    		        		  if(i<this.g.getInputs()+this.g.getOutputs()&&i!= this.g.getInputs()+this.g.getOutputs()){
    		        			  Nodes.add(600.0);
    		        			  Nodes.add(30.0+25*(i-this.g.getInputs()));
    		        			  Nodes.add(n.getValue());
    		        		  }
    		        		  else{
    		            		  if(i==this.g.getInputs()+this.g.getOutputs()){
    		              			//Bias Cell
    		              			  Nodes.add(80.0);
    		              			  Nodes.add(450.0);
    		              			  Nodes.add(n.getValue());
    		          ...
    		          for(int i=0;i<this.g.Genes.size();i++){
    		        	 Gene that = this.g.Genes.get(i);
    		        	  if(that != null&&that.isEnabled()&&that.getWeigth() != 0.0){
    		        		 ...
    		        		 if(that.getInto() > this.g.getInputs()+this.g.getOutputs()){
    		        		 ...
    		        		 if(that.getOut() > this.g.getInputs()+this.g.getOutputs()){
    		          ...
    		          String message = "MaxNeurons: "+this.g.getMaxneuron()+" Innovations: "+this.g.getInnovation();
    		          FontMetrics fontMetrics = g.getFontMetrics();
    		          int stringWidth = fontMetrics.stringWidth(message);
    		          int stringHeight = fontMetrics.getAscent();
    		          g.drawString(message, (width - stringWidth-40) / 2, (height / 2)+150 + stringHeight / 4);
    		          message =  " Generation: "+this.generation+" Fitness: "+this.g.getFitness();
    		          g.drawString(message, (width - stringWidth-40) / 2, (height / 2)+150 + stringHeight *2);

...
//		}
	...
	public int draw(Player z, int i, boolean xes) throws Exception {
		...
		switch (z.Playertype) {
		case (0):
			aiAlogrithmes.ki.visualizer.visualize(z.ki, this.path + s + " Turn: " + b + i++);
		case (3):
			aiAlgorithmes.neat.visualizer.visualize(
					z.pool.Species.get(z.pool.currentSpecies - 1).Genomes.get(z.pool.currentGenome - 1),
					this.path + s + " Turn: " + b + i++);
		case (4):
			if (z.h != null) {
				visualizer
						.visualize(
								z.h, this.path + s + " Species " + z.pool.currentSpecies + " Genome "
										+ z.pool.currentGenome + " Turn: " + b + i++,
								z.pool.currentSpecies - 1, z.pool.currentGenome - 1);
			}
		}
		return i;
	}
			...
			if (hasWon) {
				...
				if (z.ki != null) {
					...
					if (z.pool != null) {
						...
						z.receivefeedback((1000 + chips + z.pool.Species.get(z.pool.currentSpecies - 1).Genomes
								.get(z.pool.currentGenome - 1).getFitness()));// +chips
					}
				...
				if (z.ki != null) {
					...
					if (z.pool != null) {
						z.receivefeedback((-chips - 1000 + z.pool.Species.get(z.pool.currentSpecies - 1).Genomes
								.get(z.pool.currentGenome - 1).getFitness()));
					}
		...
		if (z.Playertype == 0) {
			...
			for (int j = 0; j < 6; j++) {
				for (int i = 0; i < 7; i++) {
					...
					// System.out.println("Set Node"+z.ki.Nodes.get(c)+" to
					// "+z.ki.Nodes.get(c).getValue()+" wanted "+grid[j][i]);
					c++;
				}
			}
		}

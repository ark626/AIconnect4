...
public class neatPrinter {
		...
		for (Species s : p.Species) {
			for (Genome ge : s.Genomes) {
				...
				try {
					...
					aiAlgorithmes.neat.visualizer.visualize(ge, generateImagePath(filePathPictures, null1, null2, Spec, Genome,
							"/HyperGenerator/Generator Species "));
					test.generateweigths(ge);
					visualizer.visualize(test,
							generateImagePath(filePathPictures, null1, null2, Spec, Genome, "Hyper/HN Species "),
							p.Species.indexOf(s), s.Genomes.indexOf(ge));
				} catch (Exception e) {
	...
	private static String generateImagePath(String filePathPictures, String null1, String null2, int Spec, int Genome,
			String name) {
		...
		for (Species s : p.Species) {
			for (Genome ge : s.Genomes) {
				...
				try {
					...
					file2.mkdir();
					aiAlgorithmes.neat.visualizer.visualize(ge,
							generateImagePath(filePathPictures, null1, null2, Spec, Genome, "/Neat/Species "));
	...
	public static void showFitnessEtc(Pool p) {
		...
		for (Species s : p.Species) {
			...
			System.out.println(
					"Species: " + i++ + " Average: " + s.calculateAverageFitness() + " Top:" + s.getTopFitness());
			String genomes = "";
			for (Genome g : s.Genomes) {
				genomes += j++ + ": " + g.getFitness() + " Generation " + g.getGeneration() + " Ino: "
						+ g.getInnovation() + "| ";

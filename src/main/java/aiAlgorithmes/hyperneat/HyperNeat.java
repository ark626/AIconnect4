...
public class HyperNeat {
    ...
    public String neuronPath(Neuron n, int i) {
        ...
        while (n.getOutgoing()
                .size() != 0) {
            s += "From" + i + " to " + n.getOutgoing()
                    .get(0)
                    .getInto();
            i = n.getOutgoing()
                    .get(0)
                    .getInto();
            n = Neurons[n.getOutgoing()
                    .get(0)
                    .getInto()];
    ...
    public void generateweigths(Genome CPPN) {
        ...
        for (Gene g : Links) {
            ...
            // File file = null;
            //
            // content = "Gewicht: "+this.Links.get(i++).weigth+" Von: "+g.out+"/"+g.into+"
            // "+g.out%7+"/"+(g.out/7)%6+" "+g.into%7+"/"+(g.into/7)%6+" ";
            // path += "HyperNeat.txt";
            // file = new File(path);

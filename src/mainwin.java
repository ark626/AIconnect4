...
if(testi){
	int bests=0;
	int bestg=0;
	for(Species s:test.getPool().Species){
	if(s.Genomes.contains(test.getPool().getbest())){
		bestg = 	s.Genomes.indexOf(test.getPool().getbest());
	    bests = test.getPool().Species.indexOf(s);
		test.getPool().currentSpecies = bests+1;
		test.getPool().currentGenome = bestg+1;
		test.getPool().getbest().generateNetwork();
		test.generateweigths(test.getPool().getbest());
	break;
	}
	}

	 st = "TEST Fitness vorher: "+test.getPool().getbest().getFitness();
		...
		g.run(i, dis, j);
		if(testi){
			System.out.println(st+" Fitness nachher: "+test.getPool().getbest().getFitness());
			testi = false;
		}
